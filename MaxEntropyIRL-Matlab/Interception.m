...
N = 50;
W = [0.1228 1.9574 0.37318];




[map, obst, gates, items] = LoadMap('map3.txt');
...
function f = Features(i,j, map, obst, gates)
    enGate = 4;
    distance2Goal = ([i,j]-gates(enGate,:))/14;
    f = [0; distance2Goal'];
    if j>1 && j<14 && (map(i,j-1)==1 || map(i,j+1)==1)
        f(1) = 1;
    end
end
